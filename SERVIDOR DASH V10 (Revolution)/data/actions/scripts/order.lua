...
-- TRANSFORM
        if pokeballInfo.name == "Ditto" then
			...
			local transpos = {x = 1, y = 1, z = 1}
	        if isPlayer(item2.uid) then
				doPlayerSendCancel(cid, "Voce nao Pode copiar um player!")
			return true
			end

			if getCreatureName(item2.uid) == "Ditto" then
				doPlayerSendCancel(cid, "Your ditto can't transform into another ditto.")
				markPos(summon, transpos)
			return true
			end

		
			if isMonster(item2.uid) then
				local cd = getCD(getPlayerSlotItem(cid, 8).uid, "trans", 40)

				if cd > 0 then
					doPlayerSendCancel(cid, "Your pokemon is too tired to transform again. Cooldown: ("..math.floor(cd)..")")
				return true
				end
				
				local turn = getItemAttribute(getPlayerSlotItem(cid, 8).uid, "transTurn")

				if not turn or turn > 10 then
					doItemSetAttribute(getPlayerSlotItem(cid, 8).uid, "transTurn", 0)
				else
					doItemSetAttribute(getPlayerSlotItem(cid, 8).uid, "transTurn",  turn + 1)
				end
				
				turn = getItemAttribute(getPlayerSlotItem(cid, 8).uid, "transTurn")
				
				local time = 140 + 2 * pokes[pokeballInfo.name].level
			
				setPlayerStorageValue(summon, 1010, getCreatureName(item2.uid))
				doSetCreatureOutfit(summon, getCreatureOutfit(item2.uid), -1)
				addEvent(deTransform, time * 1000, summon, turn)
				doSendMagicEffect(getThingPos(summon), 184)
				doCreatureSay(summon, "TRANSFORM!", TALKTYPE_MONSTER)
				local name = getCreatureName(item2.uid)
				setCD(getPlayerSlotItem(cid, 8).uid, "trans", 40)
				doPlayerSendCancel(cid, "@M|hide")
				doItemSetAttribute(getPlayerSlotItem(cid, 8).uid, "transBegin", os.clock())
				doItemSetAttribute(getPlayerSlotItem(cid, 8).uid, "transLeft", time)
				doItemSetAttribute(getPlayerSlotItem(cid, 8).uid, "transOutfit", getCreatureOutfit(item2.uid).lookType)
				doItemSetAttribute(getPlayerSlotItem(cid, 8).uid, "transName", getCreatureName(item2.uid))
				doPlayerSay(cid, ""..pokeballInfo.nick..", transform into "..getCreatureName(item2.uid).."!", 1)
				doUpdateCds(cid)
			return true
			end
...
-- TRANSFORM 
        ...
        if not isInArray({"Alakazam", "Tentacruel", "Shiny Tentacruel", "Haunter", "Gengar"}, pokeballInfo.name) then
        ...
        doCreatureSay(cid, ""..pokeballInfo.nick..", control "..string.lower(getCreatureName(item2.uid)).."'s mind!", 1)
        
		local cmed = item2.uid
        local cmname = pokeballInfo.name
		...
		local function check(params)
		    if isCreature(params.cid) then
	            ...
	            if not isCreature(cmed) then
                    ...
                    local pk = doSummonCreature(pokeballInfo.name, getThingPos(cid))
	            ...
	            if params.rod == 20 then
                    ...
                    doRemoveCreature(getCreatureSummons(cid)[1])
                    pk = doSummonCreature(pokeballInfo.name, getThingPos(cid))
			        if pokeballInfo.nick then
						doUpdateNick(pk, getPokeballInfo(getPlayerSlotItem(cid, 8).uid).nick.."["..getItemAttribute(getPlayerSlotItem(cid, 8).uid, "level").."]")
			        end
    ...
    if item2.uid == cid then
	    ...
	    if getPlayerStorageValue(cid, 17000) <= 0 and getPlayerStorageValue(cid, 17001) <= 0 and not isInArray(Ability['FlyRide'], pokeballInfo.name) then
	    ...
	    if getPlayerStorageValue(cid, 17001) <= 0 and getPlayerStorageValue(cid, 17000) <= 0 and isInArray(Ability['Ride'], pokeballInfo.name) then
	        local pokemon = rides[pokeballInfo.name]
	            ...
	            if isCreature(params.cid) then
	                if isCreature(getCreatureSummons(params.cid)[1]) then
	                    if getDistanceBetween(getThingPos(getCreatureSummons(cid)[1]), getThingPos(cid)) <= params.d then
						    if getCD(getPlayerSlotItem(cid, 8).uid, "AntiRide") >= 1 then
							return true
							end
	        ...
	        doCreatureSay(cid, ""..pokeballInfo.nick..", let me ride you!", 1)
            exhaustion.set(cid, etudao, 4)
	    ...
	    if getPlayerStorageValue(cid, 17001) >= 1 then
	        ...
	        local item = getPlayerSlotItem(cid, 8)
	        doCreatureSay(cid, "" .. pokeballInfo.nick..", let me get down!", 1)
	        ...
	        doCreatureAddHealth(pk, (getPlayerStorageValue(cid, 61209)))
	        doCreatureAddHealth(pk, (getPlayerStorageValue(cid, 61210))-(getPlayerStorageValue(cid, 61209)))
			setCD(getPlayerSlotItem(cid, 8).uid, "AntiRide", 5)
			doUpdateCds(cid)
	    return true
	    end
        ...
        if getPlayerStorageValue(cid, 17000) >= 1 then
	        ...
	        doCreatureAddHealth(pk, (getPlayerStorageValue(cid, 61209)))
	        doCreatureAddHealth(pk, (getPlayerStorageValue(cid, 61210))-(getPlayerStorageValue(cid, 61209)))
			setCD(getPlayerSlotItem(cid, 8).uid, "AntiFly", 5)
			doUpdateCds(cid)
        return true
        end

	    if getPlayerStorageValue(cid, 17000) <= 0 and getPlayerStorageValue(cid, 17001) <= 0 and isInArray(Ability['Fly'], pokeballInfo.name) then
	        if getPlayerStorageValue(cid, 17000) <= 0 then
	            
				local pokemon = flys[pokeballInfo.name]
	            local function fly(params)
	                if isCreature(params.cid) then
	                    if isCreature(getCreatureSummons(params.cid)[1]) then
	                        if getDistanceBetween(getThingPos(getCreatureSummons(cid)[1]), getThingPos(cid)) <= params.dd then
						        if getCD(getPlayerSlotItem(cid, 8).uid, "AntiFly") >= 1 then
							    return true
							    end
	            ...
	            doCreatureSay(cid, ""..pokeballInfo.nick..", lets fly!", 1)
                exhaustion.set(cid, etudao, 4)
...
-- ROCK SMASH
    if item2.itemid == 1285 and getPlayerStorageValue(cid, 63215) <= 0 then
	    ...
	    if not isInArray(Ability['Rock Smash'], pokeballInfo.name) then
            doPlayerSendCancel(cid, "This pokemon can't use rock smash.")
        return true
        end
            
	    if isInArray(Ability['Rock Smash'], pokeballInfo.name) then
            ...
            doPlayerSay(cid, ""..pokeballInfo.nick..", smash this rock!", 1)
...
-- CUT
    if item2.itemid == 2767 and getPlayerStorageValue(cid, 63215) <= 0 then
	    ...
	    if not isInArray(Ability['Cut'], pokeballInfo.name) then
            doPlayerSendCancel(cid, "This pokemon can't use cut.")
        return true
        end
            
	    if isInArray(Ability['Cut'], pokeballInfo.name) then
            ...
            doPlayerSay(cid, ""..pokeballInfo.nick..", cut this bush!", 1)
...
-- DIG
    if isInArray(digholes, item2.itemid) and getPlayerStorageValue(cid, 63215) <= 0 then
	    ...
	    if not isInArray(Ability['Dig'], pokeballInfo.name) then
            doPlayerSendCancel(cid, "This pokemon can't use dig.")
        return true
        end
			
        if isInArray(Ability['Dig'], pokeballInfo.name) then
            ...
            doPlayerSay(cid, ""..pokeballInfo.nick..", open this hole!", 1)
...
-- BLINK
    ...
    if not isMonster(item2.uid) and getPlayerStorageValue(cid, 63215) <= 0 and getPlayerStorageValue(cid, 17000) <= 0 then
        if isInArray(Ability['Blink'], pokeballInfo.name) then
            if exhaustion.get(cid, exhaustblink) then
	            ...
	            doPlayerSay(cid, ""..pokeballInfo.nick..", move!", 1)
                doChangeSpeed(getCreatureSummons(cid)[1], -getCreatureSpeed(getCreatureSummons(cid)[1]))
                ...
                doPlayerSay(cid, ""..pokeballInfo.nick..", teleport there!", 1)
...
-- LIGHT --
    if isCreature(item2.uid) and getCreatureMaster(item2.uid) == cid and getPlayerStorageValue(cid, 63215) <= 0 and getPlayerStorageValue(cid, 17000) <= 0 and item2.uid ~= cid then
        if not isInArray(Ability['Light'], pokeballInfo.name) then
		    local summon = getCreatureSummons(cid)[1]
			local transpos = {x = 1, y = 1, z = 1}
			if item2.uid == summon and pokeballInfo.name == "Ditto" then
				if isTransformed(summon) then
					deTransform(summon, getItemAttribute(getPlayerSlotItem(cid, 8).uid, "transTurn"))
					markPos(summon, transpos)
					doPlayerSay(cid, ""..pokeballInfo.nick..", go back to normal!", 1)
					doUpdateCds(cid)
				return true
				end
	    ...
	    if isInArray(Ability['Light'], pokeballInfo.name) then
            ...
            doPlayerSay(cid, ""..pokeballInfo.nick..", use flash!", 1)
	...
	local function move(params)
	    if isCreature(params.cid) then
	        if isCreature(getCreatureSummons(params.cid)[1]) then
	            ...
	            local dir = getDirectionTo(getThingPos(getCreatureSummons(cid)[1]), topos)
