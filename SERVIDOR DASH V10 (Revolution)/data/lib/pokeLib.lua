...
["Great Love"] = "greatlove",
["Stomp"] = "stomp",
["Night Shade"] = "nightshade",
["Cyber Pulse"] = "cyberpulse",
...
["Iceball"] = "iceball",
["Ground Chop"] = "groundchop",
["Stickmerang"] = "stickmerang",
...
["Super Vines"] = "supervines",
["Toxic Needle"] = "toxicneedle",
["Shadow Storm"] = "shadowstorm",
["Invisible"] = "invisible",
["Ice Punch"] = "icepunch",
["Psyusion"] = "psyusion",
...
["Bone Slash"] = "boneslash",
["Multi-Slap"] = "multi-slap",
["Pay Day"] = "payday",
["Earthshock"] = "earthshock",
["Thunder Fang"] = "thunderfang",
["Rage"] = "rage",
["Poisonous Wing"] = "poisonouswing",
["Body Slam"] = "bodyslam",
["Mud Bomb"] = "mudbomb",
["Sonicboom"] = "sonicboom",
...
["Shockwave"] = "shockwave",
["Healarea"] = "healarea",
["Shredder Team"] = "shredderteam",
["Bubble Blast"] = "bubbleblast",
["Reflect"] = "reflect",
["Tri Flames"] = "triflames",
...
["Fist Machine"] = "fistmachine",
["Squishy Licking"] = "squishylicking",
...
["Slime Wave"] = "slimewave",
["Aurora Beam"] = "aurorabeam",
["Solar Beam"] = "solarbeam",
["Windstorm"] = "windstorm",
["Epicenter"] = "epicenter",
["Mortal Gas"] = "mortalgas",
["Multi Punch"] = "multipunch",
["Crusher Stomp"] = "crusherstomp",
["Earthquake"] = "earthquake",
["Focus"] = "focus",
["Ancient Power"] = "ancientpower",
["Toxic Spikes"] = "toxicspikes",
["Destroyer Hand"] = "destroyerhand",
["Leaf Blade"] = "leafblade",
["Gastro Aowner"] = "gastroaowner",
["Water Oath"] = "wateroath",
["Mega Horn"] = "megahorn",
["Skull Bash"] = "skullbash",
["Hydropump"] = "hydropump",
["Future Sight"] = "futuresight",
["Wardog"] = "wardog",
["Psychicmew"] = "psychicmew",
["Psychicmewtwo"] = "psychicmewtwo",
["Raging Blast"] = "ragingblast",
["Ice Shards"] = "iceshards",
["Acid"] = "acid",
["Mega Drain"] = "megadrain",
["Electric Charge"] = "electriccharge",
["Lava Counter"] = "lavacounter",
["Helix"] = "helix",
["Melody"] = "melody",
}
...
return getPlayerStorageValue(cid, 11226)
end

function getDamagePoke(cid, shiny)
  if isSummon(cid) then
    if shiny == TRUE then
      damage = getPokemonLevel(getCreatureMaster(cid))*DamageShiny
    else
      damage = getPokemonLevel(getCreatureMaster(cid))*DamageNormal
    end
  else
    if shiny == TRUE then
      damage = tonumber(getCreatureConditionValue(cid, 8457))*DamageShiny
    else
      damage = tonumber(getCreatureConditionValue(cid, 8457))*DamageNormal
    end
  end
...
-----------------------------------------------------------------
    ...
    for i,x in pairs(pokes) do
        if i == getItemAttribute(item.uid, "poke"):sub(9, findLetter(getItemAttribute(item.uid, "poke"), "'")-1) then
			...
			if getItemAttribute(item.uid, "level") then
				level_poke = getItemAttribute(item.uid, "level")
			else
				level_poke = 1
			end
			
			if pokeballInfo.nick then
				level_nick = pokeballInfo.nick.."["..level_poke.."]"
				doUpdateNick(pk, level_nick)
			end
...
function setPokemonStatus(cid, health, vite)
	...
	if vite == true then
		local maxh = pokes[getCreatureName(cid)].cons *(((getPlayerLevel(owner) + getPokemonLevel(cid)) / 1.5) + getBoost(owner) + Gstr)
...
function doEvolueLevel(cid, poke, levelPoke)
	local noEnvolve = {Eevee, Gloom}
	if not isMonster(poke) then
    return true
    end

	if isPlayer(poke) then
        doPlayerSendCancel(poke, "Somente Pokemons!.")
    return true
    end

	if isInArray(noEnvolve, getCreatureName(poke)) then
	return true
	end

	local slo = pokesEvo[getCreatureName(poke)].maxh
	local sle = pokesEvo[getCreatureName(poke)].evolution
	local evo = pokesEvo[getCreatureName(poke)].evolution
	local pokeball = getPlayerSlotItem(cid, CONST_SLOT_FEET)

    setPokeballInfo(pokeball.uid, evo, pokesEvo[getCreatureName(poke)].maxh, pokesEvo[getCreatureName(poke)].maxh)
	doItemSetAttribute(pokeball.uid, "description", description)
	doItemSetAttribute(pokeball.uid, "happy", 250)
    doPlayerSendTextMessage(cid, 27, "Congratulations! Your "..getCreatureName(poke).." evolved into a "..evo.."!")
    ...
    doSummonPokemon(cid, sle)

	local pk = getCreatureSummons(cid)[1]
	level_nick = getCreatureName(pk).."["..levelPoke.."]"
	doUpdateNick(pk, level_nick)
    ...
    setCreatureMaxHealth(pk, getCreatureMaxHealth(pk) + (pokes[getCreatureName(pk)].cons * ((getPlayerLevel(cid) + getPokemonLevel(pk)) / 1.5)))
...
function doUpdateLevel(cid, lvl)
	local item = getPlayerSlotItem(cid, 8)
	local pokeballInfo = getPokeballInfo(getPlayerSlotItem(cid, 8).uid)
	local Summon = getCreatureSummons(cid)[1]
	local oldDir = getCreatureLookDirection(Summon)
	local oldPosPoke = getThingPos(Summon)   
	doRemoveCreature(Summon)	
	doSummonPokemon(cid, pokeballInfo.name, SummonPos)
	Summon = getCreatureSummons(cid)[1]	
	doCreatureSetNick(Summon, getCreatureName(Summon).." ["..(lvl).."]")
	setPlayerStorageValue(Summon, 11226, lvl)
	if pokeballInfo.nick then
		doCreatureSetNick(Summon, pokeballInfo.nick.." ["..(lvl).."]") 
	end

	doTeleportThing(Summon, getClosestFreeTile(Summon, oldPosPoke), false)

	if oldDir == NORTH then
		doCreatureSetLookDirection(cid, NORTH)
	elseif oldDir == EAST then
		doCreatureSetLookDirection(cid, EAST)
	elseif oldDir == SOUTH then
		doCreatureSetLookDirection(cid, SOUTH)
	elseif oldDir == WEST then
		doCreatureSetLookDirection(cid, WEST)
	end
	
	local gender = getItemAttribute(item.uid, "gender") or 0
	doCreatureSetSkullType(Summon, gender)
				
	if getItemAttribute(item.uid, "happy") then
		setPlayerStorageValue(Summon, 66604, (getItemAttribute(item.uid, "happy") or 0))
	end
	...
	if pokeballInfo.name == "Ditto" then --edited
		...
		local name = getItemAttribute(item.uid, "transName")
		if left and left > 0 then
			setPlayerStorageValue(Summon, 1010, name)
			doSetCreatureOutfit(Summon, {lookType = getItemAttribute(item.uid, "transOutfit")}, -1)
			addEvent(deTransform, left * 1000, Summon, getItemAttribute(item.uid, "transTurn"))
			doItemSetAttribute(item.uid, "transBegin", os.clock())
		else
			setPlayerStorageValue(Summon, 1010, getCreatureName(Summon) == "Ditto" and "Ditto" or "Shiny Ditto")     --edited
		end
	else
		setPlayerStorageValue(Summon, 1010, pokeballInfo.name)
	end
				
    doTeleportThing(Summon, getPlayerStorageValue(cid, 970127), false)
	doConvinceCreature(cid, Summon)
	setPokemonStatus(Summon, true, true)
	setPlayerStorageValue(cid, 61204, 1)
	doUpdateStatusPoke(cid)
	doUpdateCds(cid)
	doUpdatePokemons(cid)
	getHappinesStatus(Summon, true)
	setCD(getPlayerSlotItem(cid, 8).uid, "Anti-BugBall", 1)
	if getItemAttribute(item.uid, "isSleep") == true then
		doSleep(Summon)
	elseif getItemAttribute(item.uid, "isConfusion") == true then
